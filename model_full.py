...
class Baseline(nn.Module):
    ...
    def srgnn_preprocess(self, d, num):
        ...
        #embed()
        self.kmm_weight, MMD_dist = KMM(Z_train, Z_test, label_balance_constraints, beta=0.2)
    ...
    def loss_srgnn(self, d, criterion, args):
        ...
        target = d.y.squeeze(1).float() if args.dataset in ('proteins', 'ppi')\
            else d.y.squeeze(1).long()

        sup_loss = criterion(logits, target[train_idx])
