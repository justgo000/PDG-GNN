...
if args.method in ('erm', 'irm', 'mixup', 'groupdro', 'coral', 'dann', 'eerm', 'srgnn'):
    ...
    if args.method == 'srgnn':
        model.srgnn_preprocess(dataset, num = min(dataset.train_idx.shape[0], 5000))
elif args.method == 'ours':
    model = PDGGNN(d, c, args, device).to(device)
...
for run in range(args.runs):
    ...
    for epoch in range(args.epochs):
        ...
        if args.method == 'eerm':
            ...
            optimizer.zero_grad()
            if args.method == 'ours':
                logits, reg_loss = model(dataset.x, dataset.edge_index)
                loss = model.loss_compute(dataset, criterion, logits, reg_loss, args.lamda)
            else:
...
# with open(f"./our_loss.csv", 'a+') as write_obj:
#     for i in range(len(tr_acc)):
#         write_obj.write(f"{tr_acc[i]} " f"{val_acc[i]}\n")
...
#     write_obj.write(f"{args.method} " + f"{args.weight_decay} " + f"{args.dropout} " + f"{args.num_layers} " + f"{args.hidden_channels}: " +
#                     f"{results.mean():.2f} $\\pm$ {results.std():.2f} " + f"{train_time: .2f} " + f"{train_mem: .2f} " + f"{para_num: .2f}\n")